...
export function initPortfolioCarousel() {
  const viewport = document.getElementById("portfolio-carousel-viewport");
  const track = document.getElementById("portfolio-carousel-track");
  const dotsRoot = document.getElementById("portfolio-carousel-dots");
  ...
  /** Symmetric padding so first/last slides can snap to center with peers peeking on both sides when possible. */
...
export function updatePortfolioCarouselDotsI18n() {
  const dotsRoot = document.getElementById("portfolio-carousel-dots");
  if (!dotsRoot) {
    return;
  }
